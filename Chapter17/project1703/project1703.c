...
void update(void);
void print(void);
void erase(void);                               // proto-type of erase function.
...
// erase: Locate the node to be deleted.
...
void erase(void)
{
	...
	if (prev == NULL)
		inventory = inventory->next;          // number is in the first node
	else
		prev->next = cur->next;               // number is in some other node
	free(cur);
	return ;
}
